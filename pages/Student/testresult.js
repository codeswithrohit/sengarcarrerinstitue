...
import { 
  ...
  FaClock, 
  FaChartBar, 
  FaChartLine, // Added for Trend Chart
  FaUser,
  FaBook,
  FaCheck,
  FaTimes,
...
import { FiArrowLeft, FiChevronDown, FiTarget } from 'react-icons/fi';
import StudentNav from '../StudentNav';
import { Bar, Doughnut, Line } from 'react-chartjs-2'; // Added Line
...
const TestResult = () => {
  ...
  // Boolean to determine if we should show the trend chart (needs more than 1 attempt)
    ...
    <div className="min-h-screen bg-slate-50 pb-12 font-sans text-slate-800">
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 py-4">
        ...
        <div className="flex justify-between items-center mb-4 gap-2">
          <button onClick={() => router.push('/Student/YouTube')} className="flex items-center text-slate-600 hover:text-indigo-600 text-xs sm:text-sm font-bold bg-white px-3 py-1.5 rounded-lg shadow-sm border border-slate-200">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
          ...
          {/* Topic Analysis Bar Chart - Spans 1 or 2 columns based on trend chart presence */}
          <div className={`bg-white rounded-xl shadow-sm border border-slate-200 p-4 h-64 flex flex-col ${showTrendChart ? 'lg:col-span-1' : 'lg:col-span-2'}`}>
            ...
            <div className="flex-1 relative">
              <Bar
                data={{
                  labels: topicAnalysis.map(t => t.topic.substring(0, 10)),
